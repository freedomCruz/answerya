...
- [ ] 1.4 Create `tsconfig.base.json`: `strict`, `noUncheckedIndexedAccess`, `exactOptionalPropertyTypes`, `moduleResolution: bundler`, `isolatedModules`, `verbatimModuleSyntax`.
...
- [ ] 3.12 Create `packages/core/test/fixtures/adapter-import.fixture.ts` (excluded via `eslint.config.js` `ignores`) importing `@answerya/adapters` to prove the zone rule fires.
- [ ] 3.13 Write a Vitest test running `new ESLint().lintFiles()` programmatically against the fixture, asserting the rule reports — invoke it with the same cwd Turborepo uses for the root `lint` task, so a `basePath` regression (task 1.7) would actually be caught here, not pass silently.
